...
def search_in_list(element,element_list):
    ind = -1
    for ii, jj in enumerate(element_list):
        if jj == element:
            ind = ii
            break
    return ind
...
# Exercise-2 TODOs:
    ...
    vox = pcl_cloud_filtered.make_voxel_grid_filter()
    LEAF_SIZE = 0.01
...
def pr2_mover(object_list):
    ...
    for object in object_list:
        ...
        centroids.append([np.asscalar(cntd[0]) ,np.asscalar(cntd[1]) ,np.asscalar(cntd[2])])
